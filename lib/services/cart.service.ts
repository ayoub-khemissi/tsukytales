...
export async function addLineItem(cartId: string, variantId: number, quantity: number) {
...
export async function applyDiscount(cartId: string, code: string) {
  ...
  const context = { ...(cart.context || {}), discount_code: code, discount_rule: discount.rule };
