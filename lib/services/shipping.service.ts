...
async function getBoxtalToken(): Promise<string> {
  ...
  const response = await axios.post(`${BASE_URL}/iam/account-app/token`, {}, {
...
export async function getRelayPoints(zipCode: string, weight: number = 1.0, countryCode: string = "FR") {
  logger.info(`[Boxtal] Searching relays for ${zipCode}, country: ${countryCode}, weight: ${weight}kg`);
  const token = await getBoxtalToken();

  const response = await axios.get(`${BASE_URL}/shipping/v3.2/parcel-point-by-network`, {
    params: {
      postalCode: zipCode,
...
export async function findRelayByCode(code: string) {
  ...
  try {
        ...
        group.map((zip) =>
          axios
            .get(`${BASE_URL}/shipping/v3.2/parcel-point-by-network`, {
              params: { postalCode: zip, countryIsoCode: "FR", weight: 1, searchNetworks: "MONR_NETWORK" },
            ...
            .then((r: any) => {
              ...
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              const item = items.find((i: any) => (i.parcelPoint || i).code === code);
              if (!item) return null;
              const point = item.parcelPoint || item;
              return {
                code: point.code,
    ...
    logger.error(`[Boxtal] findRelayByCode error: ${e instanceof Error ? e.message : String(e)}`);
  }
  return null;
}
...
export async function createShipment(orderId: number) {
  ...
  const destZip = String(dest.zipCode || dest.zip_code || dest.postalCode || dest.address?.zipCode || "");
  const destCountry = (dest.country || dest.address?.country || order.metadata?.shipping_country || "FR").toUpperCase();
  ...
  const totalWeight = (order.items || []).reduce(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (sum: number, item: any) => sum + ((item.weight || 0.3) * (item.quantity || 1)),
    0,
  ) || 0.5;
  ...
  const body = {
    ...
    shipment: {
      ...
      toAddress: {
        type: "RESIDENTIAL",
        contact: { firstName, lastName, email: order.email, phone: order.metadata?.phone || "0600000000" },
        location: { street: destStreet, city: destCity, postalCode: destZip, countryIsoCode: destCountry },
      },
      ...(isRelay && dest.code ? { dropOffPointCode: dest.code } : {}),
      ...(isRelay ? { pickupPointCode: process.env.BOXTAL_PICKUP_CODE || "17199" } : {}),
    },
  };

  const response = await axios.post(`${BASE_URL}/shipping/v3.1/shipping-order`, body, {
  ...
  await orderRepository.update(orderId, {
    ...
    metadata: JSON.stringify({ ...(order.metadata || {}), shipping_order_id: shippingOrderId }),
  });
...
export async function getTrackingInfo(trackingNumber: string) {
  const steps = [
    { date: new Date(Date.now() - 86400000 * 2).toISOString(), label: "Commande préparée par Tsuky Tales", location: "Entrepôt" },
    { date: new Date(Date.now() - 86400000).toISOString(), label: "Colis remis au transporteur", location: "Centre de tri" },
    { date: new Date().toISOString(), label: "En cours de livraison", location: "Agence locale" },
  ];
  return {
...
const RELAY_COUNTRIES = ["FR", "BE", "LU", "NL", "DE", "ES", "PT", "AT", "IT", "PL"];
...
export function getShippingRates(totalWeight: number, countryCode: string = "FR") {
  ...
  const relayRates: RateTier[] = cc === "FR"
    ? [{ maxWeight: 0.5, price: 3.9 }, { maxWeight: 1, price: 4.5 }, { maxWeight: 3, price: 5.5 }, { maxWeight: 5, price: 6.9 }, { maxWeight: 10, price: 8.9 }]
    : [{ maxWeight: 0.5, price: 6.9 }, { maxWeight: 1, price: 7.9 }, { maxWeight: 3, price: 9.9 }, { maxWeight: 5, price: 12.9 }, { maxWeight: 10, price: 16.9 }];
  ...
  if (cc === "FR") {
    ...
    homeRates = [{ maxWeight: 0.5, price: 5.9 }, { maxWeight: 1, price: 6.9 }, { maxWeight: 2, price: 7.9 }, { maxWeight: 5, price: 9.9 }, { maxWeight: 10, price: 13.9 }];
  } else if (["BE", "LU", "NL", "DE", "AT"].includes(cc)) {
    homeCarrier = "Colissimo International";
    homeRates = [{ maxWeight: 0.5, price: 9.9 }, { maxWeight: 1, price: 12.9 }, { maxWeight: 2, price: 15.9 }, { maxWeight: 5, price: 19.9 }, { maxWeight: 10, price: 26.9 }];
  } else if (["ES", "PT", "IT", "GB", "IE", "CH", "PL", "CZ", "DK", "SE", "NO", "FI", "HU", "RO", "BG", "HR", "GR", "SK", "SI", "EE", "LV", "LT"].includes(cc)) {
    homeCarrier = "Colissimo International";
    homeRates = [{ maxWeight: 0.5, price: 12.9 }, { maxWeight: 1, price: 15.9 }, { maxWeight: 2, price: 19.9 }, { maxWeight: 5, price: 25.9 }, { maxWeight: 10, price: 34.9 }];
  } else if (["GP", "MQ", "GF", "RE", "YT"].includes(cc)) {
    homeCarrier = "Colissimo Outre-Mer";
    homeRates = [{ maxWeight: 0.5, price: 9.9 }, { maxWeight: 1, price: 14.9 }, { maxWeight: 2, price: 19.9 }, { maxWeight: 5, price: 29.9 }, { maxWeight: 10, price: 44.9 }];
  } else {
    homeCarrier = "Colissimo International";
    homeRates = [{ maxWeight: 0.5, price: 16.9 }, { maxWeight: 1, price: 22.9 }, { maxWeight: 2, price: 29.9 }, { maxWeight: 5, price: 42.9 }, { maxWeight: 10, price: 59.9 }];
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const result: any = {
    home: { price: findRate(homeRates), carrier: homeCarrier, label: "Domicile" },
    weight,
    country: cc,
  ...
  if (relayAvailable) {
    result.relay = { price: findRate(relayRates), carrier: "Mondial Relay", label: "Point Relais" };
  }

  return result;
}

export async function handleStatusUpdate(trackingNumber: string, newStatus: string) {
  ...
  if (newStatus === "in_transit" && order.fulfillment_status !== "shipped") {
    ...
    mailService.sendShippingNotification(order, trackingNumber).catch((err) =>
