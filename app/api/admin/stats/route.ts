...
export const GET = withErrorHandler(async () => {
  ...
  const [revenue, ordersCount, lowStock, newCustomers, salesHistory] = await Promise.all([
  ...
  const subCount = allOrders.filter((o) => (o.metadata as any)?.subscription).length;
  ...
  return NextResponse.json({
    ...
    newCustomers,
    salesHistory,
    orderTypes: { subscription: subCount, preorder: preorderCount, standard: standardCount },
  });
});
