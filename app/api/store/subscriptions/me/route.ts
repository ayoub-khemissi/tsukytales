...
export const GET = withErrorHandler(async () => {
  ...
  try {
    ...
    const product = productId ? await productRepository.findById(parseInt(productId as string)) : null;
    ...
    if (typeof priceId === "string") {
      try {
        ...
        shippingCost = totalPerQuarter - (product ? Number(product.subscription_price) : 35);
      } catch { /* ignore */ }
    }

    const shippingInfo = (customer.metadata?.subscription_shipping || {}) as Record<string, unknown>;
    ...
    return NextResponse.json({
      ...
      phases: schedule.phases.map((p) => ({
        ...
        skipped: skippedPhases.includes(new Date(p.start_date * 1000).toISOString().split("T")[0]),
      })),
    });
  } catch {
