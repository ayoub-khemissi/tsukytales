...
export async function POST(req: NextRequest) {
  ...
  if (webhookSecret) {
    ...
    try {
      if (!crypto.timingSafeEqual(new Uint8Array(Buffer.from(signature)), new Uint8Array(Buffer.from(expected)))) {
  ...
  try {
    ...
    const findOrder = async () => {
      if (shippingOrderId) {
        const found = await orderRepository.findByShippingOrderId(shippingOrderId);
    ...
    if (eventType === "TRACKING_CHANGED") {
      ...
      logger.info(`[Boxtal Webhook] Tracking update: ${trackingNumber} -> ${status}`);

      const order = await findOrder();
      if (!order) {
        logger.warn(`[Boxtal Webhook] No order found for shippingOrderId: ${shippingOrderId}`);
        return NextResponse.json({ received: true });
      }

      const meta = { ...(order.metadata || {}) };
      if (trackingNumber && !meta.tracking_number) meta.tracking_number = trackingNumber;
      if (tracking.packageTrackingUrl) meta.tracking_url = tracking.packageTrackingUrl;
      ...
      if (tracking.isFinal || status === "DELIVERED") {
        ...
        logger.info(`[Boxtal Webhook] Order TSK-${order.id} marked as delivered`);
      } else if (["ANNOUNCED", "IN_TRANSIT", "OUT_FOR_DELIVERY", "PICKED_UP"].includes(status)) {
        if (order.fulfillment_status !== "shipped" && order.fulfillment_status !== "delivered") {
          ...
          mailService.sendShippingNotification(
            { id: order.id, email: order.email, items: order.items || [], total: Number(order.total), metadata: meta },
            trackingNumber,
          ).catch((err) => logger.error("Email error: " + (err instanceof Error ? err.message : String(err))));
          logger.info(`[Boxtal Webhook] Order TSK-${order.id} marked as shipped, email sent`);
        } else {
          await orderRepository.update(order.id, { metadata: JSON.stringify(meta) });
        }
      } else {
        await orderRepository.update(order.id, { metadata: JSON.stringify(meta) });
      }
    } else if (eventType === "DOCUMENT_CREATED") {
      const labelDoc = event.payload?.documents?.find((d: { type: string }) => d.type === "LABEL") || event.payload?.documents?.[0];
      const labelUrl = labelDoc?.url;

      const order = await findOrder();
      if (order && labelUrl) {
        await orderRepository.update(order.id, {
          metadata: JSON.stringify({ ...(order.metadata || {}), label_url: labelUrl }),
        });
        logger.info(`[Boxtal Webhook] Label URL saved for TSK-${order.id}`);
      }
    }
  } catch (err) {
    logger.error(`[Boxtal Webhook] Processing error: ${err instanceof Error ? err.message : String(err)}`);
  }
